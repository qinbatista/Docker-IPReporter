...
class DDNSClient:
    def __init__(self):
        ...
        self.__google_key = "XXXXXX"
        self.__google_secret = "XXXXXXX"
    ...
    def __update_this_server(self):
        ...
        while True:
            try:
                ...
                udpClient.sendto((f"{gethostbyname(self.__target_server)},{str(self._can_connect)},{self.__google_key}:{self.__google_secret},{self._my_domain}").encode(encoding="utf-8"), (self.__target_server, 7171))
                self.__log(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}][update_this_server]Updated to server={gethostbyname(self.__target_server)}, {self.__target_server}: reachable={self._can_connect} message: {this_docker_ip},{str(self._can_connect)},{self.__google_key}:{self.__google_secret},{self._my_domain}")
                time.sleep(60)
            except Exception as e:
